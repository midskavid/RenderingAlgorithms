...
Matrix4x4 Inverse(const Matrix4x4 &m) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        indxr[i] = irow;
        indxc[i] = icol;
        if (minv[icol][icol] == 0.f) std::cout<<"Singular matrix in MatrixInvert\n";
...
Transform LookAt(const Point3f &pos, const Point3f &look, const Vector3f &up) {
    ...
    if (Cross(Normalize(up), dir).Length() == 0) {
        std::cout<<"Up and View are in same direction..\n";
        return Transform();
    }
...
inline Interval Sin(const Interval &i) {
    if (i.low<0) std::cout<<"wrong interval..\n";
    if (i.high>2.0001 * Pi) std::cout<<"wrong interval..\n";
